...
final class MacroListScreen extends Screen {
      ...
      try {
         ...
         for (int macroNumber = startMacro; macroNumber <= endMacro; macroNumber++) {
            ...
            this.addRenderableWidget(Button.builder(Component.literal(this.fit(label, listWidth - 266)), button -> {
               ...
               this.minecraft.setScreenAndShow(new MacroListScreen(this.runner, this.page));
            }).bounds(left, rowY, listWidth - 256, 20).build());
            this.addRenderableWidget(Button.builder(Component.literal("Edit"), button -> {
               this.runner.selectMacro(this.minecraft, selectedMacro);
               this.minecraft.setScreenAndShow(new MacroBuilderScreen(this.runner, this.runner.model()));
         ...
         ((Button)this.addRenderableWidget(
               Button.builder(Component.literal("Prev"), button -> this.minecraft.setScreenAndShow(new MacroListScreen(this.runner, this.page - 1)))
                  .bounds(left, footerY, 58, 20)
                  .build()
            ))
            .active = this.page > 0;
         ((Button)this.addRenderableWidget(
               Button.builder(Component.literal("Next"), button -> this.minecraft.setScreenAndShow(new MacroListScreen(this.runner, this.page + 1)))
                  .bounds(left + 66, footerY, 58, 20)
                  .build()
            ))
            .active = endMacro < this.runner.macroCount();
         this.addRenderableWidget(
            Button.builder(Component.literal("Edit Selected"), button -> this.minecraft.setScreenAndShow(new MacroBuilderScreen(this.runner, this.runner.model())))
         ...
         this.addRenderableWidget(
            Button.builder(
                  Component.literal("Import"), button -> this.minecraft.setScreenAndShow(new MacroImportScreen(this.runner, this.runner.currentMacroNumber(), this.page))
               )
         ...
         this.addRenderableWidget(
            Button.builder(
                  ...
                  button -> this.minecraft.setScreenAndShow(new MacroResetConfirmScreen(this.runner, this.runner.currentMacroNumber(), this.page))
               )
               .bounds(toolLeft + 288, toolY, 58, 20)
               .build()
         );
         this.addRenderableWidget(
            Button.builder(Component.literal("Marketplace"), button -> this.minecraft.setScreenAndShow(new MacroMarketplaceScreen(this.runner, this.page)))
   ...
   public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
      ...
      try {
         super.extractRenderState(context, layoutMouseX, layoutMouseY, delta);
         context.centeredText(this.font, this.title, this.width / 2, 28, -1);
         context.centeredText(this.font, Component.literal("Page " + (this.page + 1) + " / " + this.totalPages()), this.width / 2, 44, -4204545);
         context.centeredText(this.font, Component.literal("Selected: Macro " + this.runner.currentMacroNumber()), this.width / 2, 58, -1);
         ...
         context.centeredText(this.font, Component.literal(this.fit(this.runner.status(), this.width - 32)), this.width / 2, this.height - 52, -2565928);
         context.centeredText(
   ...
   public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
      ...
      try {
         return super.mouseClicked(this.layoutClick(click, scale), doubled);
      } finally {
         this.restoreActualSize(actualWidth, actualHeight);
      }
   }
   ...
   public boolean mouseReleased(MouseButtonEvent click) {
      ...
      try {
         return super.mouseReleased(this.layoutClick(click, scale));
      } finally {
         this.restoreActualSize(actualWidth, actualHeight);
      }
   }
   ...
   public boolean mouseDragged(MouseButtonEvent click, double offsetX, double offsetY) {
      ...
      try {
         return super.mouseDragged(this.layoutClick(click, scale), offsetX / scale, offsetY / scale);
      } finally {
         this.restoreActualSize(actualWidth, actualHeight);
      }
   }
   ...
   public boolean keyPressed(KeyEvent input) {
      if (this.bindingMacroNumber <= 0) {
         ...
         if (input.key() != 256 && input.key() != 259 && input.key() != 261) {
            this.runner.setMacroKey(this.bindingMacroNumber, input.key());
         } else {
