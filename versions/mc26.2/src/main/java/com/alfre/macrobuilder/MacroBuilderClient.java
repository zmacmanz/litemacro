...
public final class  MacroBuilderClient implements ClientModInitializer {
   ...
   private static final Category KEY_CATEGORY = Category.register(Identifier.fromNamespaceAndPath("macro_builder", "main"));
   ...
   private static void registerHudOverlay() {
      HudElementRegistry.addLast(Identifier.fromNamespaceAndPath("macro_builder", "running_overlay"), (context, tickCounter) -> renderMacroOverlay(context, Minecraft.getInstance()));
   }

   private static void renderMacroOverlay(GuiGraphicsExtractor context, Minecraft client) {
      if (client == null || client.player == null || client.level == null || !RUNNER.running()) {
         return;
      }

      if (client.gui.screen() instanceof MacroListScreen || client.gui.screen() instanceof MacroBuilderScreen || client.gui.screen() instanceof MacroMarketplaceScreen) {
         return;
      }
      ...
      context.text(client.font, "Litemacro Running", x + 8, y + 6, 0xFFEFF6FF);
      ...
      for (int index = 0; index < visibleRows; index++) {
         ...
         context.text(client.font, fitOverlay(client, "#" + snapshot.macroNumber() + " " + snapshot.macroName(), panelWidth - 24), x + 14, rowY + 2, 0xFFEFF6FF);
         String detail = snapshot.componentName() + "  " + Math.max(0, snapshot.ticks() / 20) + "s";
         context.text(client.font, fitOverlay(client, detail, panelWidth - 24), x + 14, rowY + 14, 0xFF9BAFC2);
         rowY += 27;
      }

      if (snapshots.size() > visibleRows) {
         context.text(client.font, "+" + (snapshots.size() - visibleRows) + " more", x + 14, y + panelHeight - 10, 0xFF6F8192);
      }
   }
   ...
   private static void registerKeybind() {
      openKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("key.macro_builder.open", Type.KEYSYM, 66, KEY_CATEGORY));
   ...
   private static void registerScreenEvents() {
      ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
         ...
         ScreenKeyboardEvents.afterKeyPress(screen).register((currentScreen, input) -> {
            handleMenuKeyPress(client, input);
         });
   ...
   private static void handleMenuKeyPress(Minecraft client, KeyEvent input) {
      if (openKey != null && openKey.matches(input)) {
         openMacroList(client, true);
      } else {
         RUNNER.handleMacroHotkeyPress(client, input.key());
      }
   }

   private static boolean canOpenMacroList(Minecraft client) {
      if (client == null || client.gui.screen() instanceof MacroListScreen || client.gui.screen() instanceof MacroBuilderScreen) {
         return false;
      }

      return client.gui.screen() == null || client.player == null || client.level == null;
   }
   ...
   private static void openMacroList(Minecraft client, boolean force) {
      if ((force && client != null && !(client.gui.screen() instanceof MacroListScreen) && !(client.gui.screen() instanceof MacroBuilderScreen)) || canOpenMacroList(client)) {
         client.setScreenAndShow(new MacroListScreen(RUNNER));
      }
   }
   ...
   private static LiteralArgumentBuilder<FabricClientCommandSource> commandRoot(String commandName) {
      return (LiteralArgumentBuilder<FabricClientCommandSource>)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)ClientCommands.literal(
                                                                  commandName
                                                               )
                                                               .executes(context -> openMacroScreen()))
                                                            .then(ClientCommands.literal("help").executes(context -> showCommandHelp())))
                                                         .then(ClientCommands.literal("list").executes(context -> showCommandHelp())))
                                                      .then(ClientCommands.literal("open").executes(context -> openMacroScreen())))
                                                   .then(ClientCommands.literal("macro1").executes(context -> selectMacro(1))))
                                                .then(ClientCommands.literal("macro2").executes(context -> selectMacro(2))))
                                             .then(
                                                ClientCommands.literal("macro")
                                                   .then(
                                                      ClientCommands.argument("number", IntegerArgumentType.integer(1, 100))
                                                         .executes(context -> selectMacro(IntegerArgumentType.getInteger(context, "number")))
                                                   )
                                             ))
                                          .then(
                                             ClientCommands.literal("select")
                                                .then(
                                                   ClientCommands.argument("number", IntegerArgumentType.integer(1, 100))
                                                      .executes(context -> selectMacro(IntegerArgumentType.getInteger(context, "number")))
                                                )
                                 ...
                                 .then(ClientCommands.literal("resume").executes(context -> resumeMacro())))
                              .then(ClientCommands.literal("stop").executes(context -> stopMacro())))
                           .then(ClientCommands.literal("reload").executes(context -> reloadMacro())))
                        .then(
                           ClientCommands.literal("copy")
                              .then(
                                 ClientCommands.argument("number", IntegerArgumentType.integer(1, 100))
         ...
         .then(
            ((LiteralArgumentBuilder)((LiteralArgumentBuilder)ClientCommands.literal("chat")
                     .then(ClientCommands.literal("on").executes(context -> setChatNotifications(true))))
                  .then(ClientCommands.literal("off").executes(context -> setChatNotifications(false))))
               .then(ClientCommands.literal("toggle").executes(context -> toggleChatNotifications()))
         );
   }
   ...
   private static void sendClientMessage(String message) {
      ...
      if (client.player != null) {
         client.player.sendSystemMessage(Component.literal("[Litemacro] " + message));
      }
   }
